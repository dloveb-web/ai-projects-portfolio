...
CAT_EMBED_DIM = 24
BASE_PATH = ''
MODEL_DIR = 'dnn_v4_models/'
os.makedirs(BASE_PATH + MODEL_DIR, exist_ok=True)
...
# ========== 异常值处理 ==========
# 删除 power=0 的训练样本（8.5%，可能是数据错误）
p0_cnt = (train['power'] == 0).sum()
train = train[train['power'] > 0].reset_index(drop=True)
print(f"删除 power=0: {p0_cnt} 条")

# 测试集 power=0 用品牌中位数填充
brand_mp = train.groupby('brand')['power'].median()
global_power = train['power'].median()
mask = test['power'] == 0
test.loc[mask, 'power'] = test.loc[mask, 'brand'].map(brand_mp).fillna(global_power).astype('int64')

# ========== 特征工程 ==========
test['price'] = -1
data = pd.concat([train, test], axis=0, ignore_index=True)
n_train = len(train)
print(f"训练集: {n_train}, 测试集: {len(test)}")
...
data['notRepairedDamage'] = pd.to_numeric(data['notRepairedDamage'], errors='coerce').fillna(0)  # 190 NaN → 0
data['bodyType'] = data['bodyType'].fillna(-1)  # 2057 NaN → -1
data['model'] = data['model'].fillna(-1)  # 1 NaN → -1
...
for seed_idx, SEED in enumerate(SEEDS):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_numeric, price_bins)):
        print(f"\n  Fold {fold+1}/{N_FOLDS}")
    ...
    # 保存最佳模型
    torch.save(best_state, BASE_PATH + MODEL_DIR + f'seed{SEED}_fold{fold}.pt')
    print(f"    模型已保存: {MODEL_DIR}seed{SEED}_fold{fold}.pt")
