...
for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
    ...
    print(f"Stacking MAE: {stack_mae:.2f} (Cat={meta_model.coef_[0]:.3f}, LGB={meta_model.coef_[1]:.3f})")
...
print(f"Stacking 平均 MAE: {np.mean(fold_maes_stack):.2f} ± {np.std(fold_maes_stack):.2f}")
...
submit.to_csv('sota_final_fixed_submit.csv', index=False)
print(f"\n✅ 结果已保存: sota_final_fixed_submit.csv")
