...
for df in [train, test]:
    ...
    df['car_age'] = (df['creatDate'] // 10000 - df['reg_year']).clip(lower=0)
...
X_test = X_test[[col for col in numeric_cols if col in test_clean.columns]]
...
# 平均权重
cat_weight = 1 / (1 / np.mean(all_maes['cat']))
lgb_weight = 1 / (1 / np.mean(all_maes['lgb']))
print(f"\n平均融合权重: CatBoost={cat_weight:.3f}, LightGBM={lgb_weight:.3f}")
