...
# 自适应融合权重
final_inv_maes = [1/np.mean(cat_maes), 1/np.mean(lgb_maes)]
final_total = sum(final_inv_maes)
final_weights = [w/final_total for w in final_inv_maes]
...
submit.to_csv('reliable_final_submit.csv', index=False)
print(f"\n结果已保存到 reliable_final_submit.csv")
