# -*- coding: utf-8 -*-
"""
二手车价格预测 - 增强特征工程版 (方案1 - 简化版)
去掉 LightGBM，使用 CatBoost + XGBoost + NeuralNet 三模型融合
目标：MAE <= 450
优化内容：
...
def main():
    print("\n" + "="*60)
    print("方案1：增强特征工程 + XGBoost + 自适应融合 (简化版)")
    print("="*60)
    ...
    output_file = 'enhanced_v1_simple_submit.csv'
    ...
    plt.title('方案1：增强特征工程 + XGBoost + 自适应融合 (简化版)', fontsize=14, fontweight='bold')
    plt.grid(axis='y', alpha=0.3)
    plt.tight_layout()
    plt.savefig('enhanced_v1_simple_comparison.png', dpi=150)
    plt.close()
    print(f"✓ 性能对比图已保存: enhanced_v1_simple_comparison.png")
