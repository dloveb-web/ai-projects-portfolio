...
print("="*60)
print("终极修复版 - 基于480.64成功策略 + 完全正确的参数")
print("目标: MAE <= 450")
print("="*60)
...
print(f"CatBoost参数: {catboost_params}")
print(f"LightGBM参数: {lightgbm_params}")
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
    ...
    lgb_model = lgb.train(
        **lightgbm_params,
        train_data_lgb, num_boost_round=catboost_params['iterations'],
...
submit.to_csv('ultimate_fix_submit.csv', index=False)
print(f"\n结果已保存到 ultimate_fix_submit.csv")
...
print(f"✅ 完全正确的参数（CatBoost使用l2_leaf_reg, LightGBM使用learning_rate）")
print(f"✅ 自适应权重融合")
print(f"✅ 5折交叉验证")
print(f"✅ 早停机制（CatBoost:130轮, LightGBM:130轮）")
