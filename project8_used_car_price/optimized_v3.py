...
二手车价格预测 - 优化v3版
目标: 测试集MAE < 450
基于: optimized_v2 (MAE 475.68) 进一步优化
策略: 更多特征 + 更激进参数 + 最佳权重
"""
...
print("="*70)
print("优化v3版 - 目标 MAE < 450")
print("基于: optimized_v2 (MAE 475.68)")
print("="*70)
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
    ...
    # 测试多种权重
    # 权重1: Cat:35%, Lgb:65%
    avg_pred_w1 = 0.35 * cat_pred_val + 0.65 * lgb_pred_val
    mae_w1 = mean_absolute_error(y_val, avg_pred_w1)
    fold_maes_w1.append(mae_w1)
    print(f"  权重(35:65) MAE: {mae_w1:.2f}")

    # 权重2: Cat:30%, Lgb:70%
    avg_pred_w2 = 0.30 * cat_pred_val + 0.70 * lgb_pred_val
    mae_w2 = mean_absolute_error(y_val, avg_pred_w2)
    fold_maes_w2.append(mae_w2)
    print(f"  权重(30:70) MAE: {mae_w2:.2f}")

    # 权重3: Cat:25%, Lgb:75%
    avg_pred_w3 = 0.25 * cat_pred_val + 0.75 * lgb_pred_val
    mae_w3 = mean_absolute_error(y_val, avg_pred_w3)
    fold_maes_w3.append(mae_w3)
    print(f"  权重(25:75) MAE: {mae_w3:.2f}")
...
print(f"\n权重(35:65) MAE: {np.mean(fold_maes_w1):.2f} ± {np.std(fold_maes_w1):.2f}")
print(f"权重(30:70) MAE: {np.mean(fold_maes_w2):.2f} ± {np.std(fold_maes_w2):.2f}")
print(f"权重(25:75) MAE: {np.mean(fold_maes_w3):.2f} ± {np.std(fold_maes_w3):.2f}")

# 选择最佳权重
best_weights = [
    ('Cat:35%, Lgb:65%', np.mean(fold_maes_w1), 0.35, 0.65),
    ('Cat:30%, Lgb:70%', np.mean(fold_maes_w2), 0.30, 0.70),
    ('Cat:25%, Lgb:75%', np.mean(fold_maes_w3), 0.25, 0.75)
]
best_weight = min(best_weights, key=lambda x: x[1])
final_mae = best_weight[1]

print(f"\n{'='*70}")
print(f"最佳权重: {best_weight[0]}")
...
# 使用最佳权重
final_pred = best_weight[2] * cat_test_preds + best_weight[3] * lgb_test_preds
final_pred = np.maximum(final_pred, 50)

# 后处理
mean_pred = final_pred.mean()
if mean_pred > 6000:
    adjustment_factor = 0.980
...
print(f"训练模型数: 2")
print(f"5折交叉验证")
print(f"融合策略: 加权平均 ({best_weight[0]})")
