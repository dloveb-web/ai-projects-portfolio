...
# ==================== 数据预处理 ====================
def preprocess_data(X_train, y_train, X_val, y_val, cat_features):
    """
    数据预处理：去除异常值、填充缺失值、标准化数值特征、处理分类特征中的NaN
    """
    # 去除目标变量中的极端异常值（如超过3倍标准差）
    threshold = y_train.std() * 3
    mask = (y_train <= y_train.mean() + threshold) & (y_train >= y_train.mean() - threshold)
    X_train = X_train[mask]
    y_train = y_train[mask]

    # 填充缺失值（数值型用均值，分类型用默认值）
    ...
    for col in cat_features:
        if col in X_train.columns and col in X_val.columns:
            ...
            print(f"警告：分类特征 '{col}' 不存在于数据中，已跳过处理。")
    ...
    return X_train, y_train, X_val, y_val, valid_cat_features


# ==================== 特征工程 ====================
def engineer_features(X_train, X_val):
    """
    特征工程：分析v0-v14之间的相关性并处理高度相关特征
    """
    # 计算特征之间的相关性矩阵
...
def predict_test_data(cat_features):
    """
    预测测试集数据
    """
    print("\n正在加载测试数据...")
    ...
    for col in cat_features:
        if col in test_data.columns:
            ...
            print(f"警告：分类特征 '{col}' 不存在于测试数据中，已跳过处理。")
    ...
    # 创建数据池并预测
    test_pool = Pool(test_data, cat_features=valid_cat_features)
...
def main():
    ...
    # 数据预处理
    X_train_clean, y_train_clean, X_val_clean, y_val_clean, valid_cat_features = preprocess_data(
        X_train_raw, y_train_raw, X_val_raw, y_val_raw, cat_features
    )

    # 特征工程
    X_train_eng, X_val_eng = engineer_features(X_train_clean, X_val_clean)

    # 训练模型
    model = train_catboost_model(X_train_eng, X_val_eng, y_train_clean, y_val_clean, valid_cat_features)

    # 评估模型
    rmse, mae, r2 = evaluate_model(model, X_val_eng, y_val_clean, valid_cat_features)

    # 绘制特征重要性
    plot_feature_importance(model, X_train_eng)

    # 预测测试集
    predict_test_data(valid_cat_features)
