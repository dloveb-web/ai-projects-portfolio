# -*- coding: utf-8 -*-
"""
二手车价格预测 - 中位数填充异常值 + 优化特征工程版
目标：MAE <= 450
优化内容：
1. 中位数填充异常值（功率、里程、价格、v特征）
2. 增强特征工程
3. CatBoost + XGBoost + NeuralNet 三模型融合
4. 自适应权重融合
"""
...
plt.rcParams['font.sans-serif'] = ['Arial']  # 使用Arial避免字体问题
...
def enhanced_feature_engineering(train, test):
    ...
    if len(available_v_cols) >= 3:
        ...
        triad_pairs = [('v_0', 'v_2', 'v_3'), ('v_0', 'v_3', 'v_12'), ('v_0', 'v_2', 'v_12')]
...
def main():
    print("\n" + "="*60)
    print("中位数填充异常值 + 增强特征工程 + XGBoost + 自适应融合")
    print("="*60)
    ...
    common_numeric_cols = [c for c in numeric_cols if c in test_numeric_cols]
    test[common_numeric_cols] = test[common_numeric_cols].fillna(X[common_numeric_cols].median())
    ...
    output_file = 'median_fill_optimized_submit.csv'
    ...
    plt.title('中位数填充异常值 + 优化特征工程', fontsize=14, fontweight='bold')
    plt.grid(axis='y', alpha=0.3)
    plt.tight_layout()
    plt.savefig('median_fill_comparison.png', dpi=150)
    plt.close()
    print(f"✓ 性能对比图已保存: median_fill_comparison.png")
